...
namespace Lib_D2Data.Equipment
{
    public class Weapon : Equipment
    {
        ...
        public static void Import(string excelFolder)
        {
            ...
            WeaponTypes = new Dictionary<string, string>();

            var table = D2Data.ReadTxtFileToDictionaryList(excelFolder + "/Weapons.txt");
            ...
            foreach (var row in table)
            {
                ...
                if (!WeaponTypes.ContainsKey(ItemType.ItemTypes[row["type"]].Name))
                {
                    WeaponTypes[row["code"]] = ItemType.ItemTypes[row["type"]].Name;
                }
