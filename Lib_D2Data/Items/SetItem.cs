...
namespace Lib_D2Data.Items
{
    public class SetItem : Item
    {
        ...
        public static void Import(string excelFolder)
        {
            ...
            var table = D2Data.ReadTxtFileToDictionaryList(excelFolder + "/SetItems.txt");
            ...
            foreach (var row in table)
            {
                ...
                for (int i = 1; i <= 9; i++)
                {
                    propList.Add(new Types.PropertyInfo(row[$"prop{i}"], row[$"par{i}"], row[$"min{i}"], row[$"max{i}"]));
                }
                ...
                for (int i = 1; i <= 5; i++)
                {
                    propList.Add(new Types.PropertyInfo(row[$"aprop{i}a"], row[$"apar{i}a"], row[$"amin{i}a"], row[$"amax{i}a"]));
                    propList.Add(new Types.PropertyInfo(row[$"aprop{i}b"], row[$"apar{i}b"], row[$"amin{i}b"], row[$"amax{i}b"]));
                }
        ...
        private static void SetPartialPropertyString(SetItem setItem)
        {
            ...
            if (setItem.SetProperties == null) return;
