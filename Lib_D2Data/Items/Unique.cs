...
namespace Lib_D2Data.Items
{
    public class Unique : Item
    {
        ...
        public static List<Unique> Import(string excelFolder)
        {
            ...
            var table = D2Data.ReadTxtFileToDictionaryList(excelFolder + "/UniqueItems.txt");
            ...
            foreach (var row in table)
            {
                ...
                try
                {
                    var properties = Types.ItemProperty.GetProperties(propList, unique.ItemLevel).OrderByDescending(x => x.ItemStatCost == null ? 0 : x.ItemStatCost.DescriptionPriority).ToList();
