...
namespace Lib_D2Data.Items
{
    public class Runeword : Item
    {
        ...
        public static List<Runeword> Import(string excelFolder)
        {
            ...
            var table = D2Data.ReadTxtFileToDictionaryList(excelFolder + "/Runes.txt");
            ...
            foreach (var row in table)
            {
                ...
                for (int i = 1; i <= 7; i++)
                {
                    propList.Add(new Types.PropertyInfo(row[$"T1Code{i}"], row[$"T1Param{i}"], row[$"T1Min{i}"], row[$"T1Max{i}"]));
                }
